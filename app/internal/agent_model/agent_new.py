from app.internal.agent_model import agent_data
from app.internal.agent_model.tokenizer import count_tokens
...
###This new agent class is designed to be self-persistent unless terminated, implement the case of time and state dependent agent
...
#wrapper class for original agent class to be ran as a background process once instantiated
...
#define a pydantic class for agent state representations for each iteration
#can be interpreted as a snapshot of the Agent instance at that point in time
class AgentState(BaseModel):
    agent_id: str
    lifespan: int #max allowed iterations
    elapsed_lifespan: int ##the current iteration timestep of the agent, range 0 to n
    agent_memory: str  ##the information state constructed from the iteration
    input: Optional[str] = None
    output: Optional[str] = None

    def update_state(self, agent: Agent, query: str, response: str):
        pass
...
#####will definitely need a finetuned model to enable self continuation of "thought" type persistency


#####Datastr compression algorithm for agent_data once exceeded by removing chunks deviating from principal components of the datastr


##annealing type self persistency, constructs 
class PersistentAgent:
    #false if persistency lifespan not specified 
    def __init__(self, agent: Agent, lifespan: Optional[int] = None):
        self.agent: Agent = agent
        self.lifespan: int = lifespan
        self.states: list[AgentState] = []        #list of AgentState
        self.current_state: AgentState = AgentState(
            agent_id = str(uuid.uuid4()),
            timestep = 0,
            
        )
        self.agent_id = str(uuid.uuid4())
        self.state_storage = database["agent_state"]  #agent_state collection in the database


    #defining self.methods


    #perform a query on the agent 
    def query(self, input_string: str):
        pass

    #should be writing agent states to some persistent db
